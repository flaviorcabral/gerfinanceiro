<?php
    ...
    if(isset($_REQUEST['pagar'])){
        $contas = $con->listaContas('pagar');
        $tipo = 'Pagar';
        $_SESSION['tipo'] = $tipo;
    }

    if(isset($_REQUEST['receber'])){
        $contas = $con->listaContas('receber');
        $tipo = 'Receber';
        $_SESSION['tipo'] = $tipo;

    }

    if($_SESSION['tipo'] == "Receber"){
        $contas = $con->listaContas('receber');
    }

    if($_SESSION['tipo'] == "Pagar"){
        $contas = $con->listaContas('pagar');
    }
?>
<!DOCTYPE html>
                    ...
                    <div class="ibox float-e-margins">
                        <div class="ibox-title">
                            <button type="button" class="btn btn-warning btn-lg btn-block"><i
                                                        class="fa fa-money"></i> Contas a <?php echo $_SESSION['tipo']; ?>
                            </button>
                        </div>

                        <div class="ibox-content">
                            <div class="pull-left">
                                <a onclick="#" href="contasacoes.php?acao=nova&tipo=<?php echo $_SESSION['tipo']; ?>" class="btn btn-warning ">Incluir</a>
                            </div>
                            <br>
                            <br>
                            <br>
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover dataTables-example" >
                                    <thead>
                                        <tr>
                                            <th class="col-lg-1">Venda/Pedido</th>
                                    ...
                                    <tbody>
                                        <?php for ($f =0; $f <= (count($contas)-1); $f++) { ?>
                                            <tr class="gradeX">
                                                ...
                                                <?php $coluna5 = "R$ ".number_format($contas[$f]["vlcont"],2,',','.'); ?>
                                                <?php $coluna6 = ""; ?>
                                                <?php $coluna7 = ""; ?>
